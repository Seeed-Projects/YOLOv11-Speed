...
def run_inference_pipeline(net, input, batch_size, labels, output_dir,
    ...
    possible_paths = [
        os.path.join(os.path.dirname(__file__), "config", "config.json"),  # Relative to this file
        os.path.join(os.path.dirname(os.path.dirname(__file__)), "src", "config", "config.json"),  # From project root
        "src/config/config.json",  # Direct path from project root
    ]
